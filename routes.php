...
Route::get('frontend/user/{provider}/{action?}', array("as" => "sociallogin_provider", function($provider_name, $action = ""){
  $success_redirect = Input::get('s', '/');
  $error_redirect = Input::get('f', '/login');

  $manager = Frontend\User\Classes\ProviderManager::instance();
  ...
  $user = \Frontend\User\Classes\UserManager::instance()->find(
    $provider_details,
    $user_details
  );
